...
if(isset($_POST['payer'])){
	...
	if(empty($error)){
		...
		// vider le panier 
		unset($_SESSION['panier']);
		$_SESSION['validation'][] = 'Votre commande a bien été prise en compte. Voici le numéro de commande <b>N°'. date('Y') . '-' . $id_commande . '</b><br/>Un email de confirmation vient de vous être envoyé à l\'adresse <u>' . $_SESSION['membre']['email'] . '</u>'; 			
		header('location:' . PATH . 'panier.php');
		exit; 
	...
	<tbody align="center">
		<?php if(count($_SESSION['panier']['id_produit']) > 0) : ?>
			...
			<tr>
				...
				<a href="?action=decrementation&id=<?= $_SESSION['panier']['id_produit'][$i] ?>"><i class="text-info fas fa-minus-square"></i></a>
				&nbsp;
				<?= $_SESSION['panier']['quantite'][$i] ?>
				&nbsp;
				<a href="?action=incrementation&id=<?= $_SESSION['panier']['id_produit'][$i] ?>"><i class="text-info fas fa-plus-square"></i></a>
			...
			<tr>
				<td colspan="6">	
					<form method="post" action="">
						<input type="submit" class="btn btn-success" name="payer" value="Payer le panier"/>
					</form>
				</td>
			</tr>
		<?php else :  ?>
